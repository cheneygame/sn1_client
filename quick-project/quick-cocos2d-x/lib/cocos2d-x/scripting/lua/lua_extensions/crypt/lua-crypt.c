...
#undef luaL_addsize
#define luaL_addsize(b , len) char* temp = malloc(len) ;lua_pushlstring(L, temp , len) ; free(temp)
#define luaL_pushresult(b) 
#define luaL_checkversion(L)
//#define luaL_buffinitsize(L, B, sz) luaL_buffinit(L,B) ; luaL_prepbuffsize(B, sz)
#define luaL_buffinitsize(L, B, sz) malloc(sz)
#else
...
lb64decode(lua_State *L) {
	...
	for (i=0;i<sz;) {
		...
		for (j=0;j<4;) {
			...
			c[j] = b64index(text[i]);
			//printf("lb64decode:i,j,c[j], text[i]:%d,%d,%d,%d\n",i,j, c[j], text[i]);
			if (c[j] == -1) {
				++i;
				continue;
			}
