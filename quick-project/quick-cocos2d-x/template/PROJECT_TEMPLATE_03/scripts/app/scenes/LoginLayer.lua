...
function LoginLayer:ctor()
	...
	self.nameEditBox:setText("111113")
	self.passEditBox:setText("123456")
	...
	local function onClicked(tag)  
        if tag == 1 then  
        	...
        	DataManager.setAccount(tostring(self.nameEditBox:getText()))
			-----------------------------------------------------------
			-----------------------------------------------------------
			--[[
			...
			print("send message",message:getLen(),message:toString())
			]]
   			-----------------------------------------------------------
   			local name = "login"
   			local args = {user = tostring(self.nameEditBox:getText()), pass = self.passEditBox:getText(),server = "sample"}
   			socketManager:send_sproto(CMD_LOGIN,name, args)
    		--socketManager:sendMessage(message)	
        end
